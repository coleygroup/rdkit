...
TEST_CASE_METHOD(SmartsAnalyzerFixture, "SmartsAnalyzer: add_atom_maps simple",
...
TEST_CASE_METHOD(SmartsAnalyzerFixture, "SmartsAnalyzer: calculate_dof simple",
                 "[SmartsAnalyzer]") {
...
TEST_CASE_METHOD(SmartsAnalyzerFixture,
                 "SmartsAnalyzer: calculate_dof single OR",
                 "[SmartsAnalyzer]") {
...
TEST_CASE_METHOD(SmartsAnalyzerFixture,
                 "SmartsAnalyzer: calculate_dof multiple OR",
                 "[SmartsAnalyzer]") {
  // [C,N][C,O] = 2 * 2 = 4
...
TEST_CASE_METHOD(SmartsAnalyzerFixture, "SmartsAnalyzer: calculate_dof AND",
                 "[SmartsAnalyzer]") {
...
TEST_CASE_METHOD(SmartsAnalyzerFixture, "SmartsAnalyzer: calculate_dof NOT",
                 "[SmartsAnalyzer]") {
...
TEST_CASE_METHOD(SmartsAnalyzerFixture, "SmartsAnalyzer: calculate_dof complex",
                 "[SmartsAnalyzer]") {
...
// Test bond queries
TEST_CASE_METHOD(SmartsAnalyzerFixture, "SmartsAnalyzer: calculate_dof bonds",
                 "[SmartsAnalyzer]") {
...
TEST_CASE_METHOD(SmartsAnalyzerFixture, "SmartsAnalyzer: calculate_dof invalid",
                 "[SmartsAnalyzer]") {
...
// Test ring queries
TEST_CASE_METHOD(SmartsAnalyzerFixture, "SmartsAnalyzer: calculate_dof rings",
                 "[SmartsAnalyzer]") {
...
TEST_CASE_METHOD(SmartsAnalyzerFixture,
                 "SmartsAnalyzer: enumerate_variants simple",
                 "[SmartsAnalyzer]") {
...
// Test SMARTS with single OR query
TEST_CASE_METHOD(SmartsAnalyzerFixture,
                 "SmartsAnalyzer: enumerate_variants single OR",
                 "[SmartsAnalyzer]") {
...
TEST_CASE_METHOD(SmartsAnalyzerFixture,
                 "SmartsAnalyzer: enumerate_variants three-way OR",
                 "[SmartsAnalyzer]") {
...
TEST_CASE_METHOD(SmartsAnalyzerFixture,
                 "SmartsAnalyzer: enumerate_variants multiple OR",
                 "[SmartsAnalyzer]") {
...
TEST_CASE_METHOD(SmartsAnalyzerFixture,
                 "SmartsAnalyzer: enumerate_variants max limit",
                 "[SmartsAnalyzer]") {
...
TEST_CASE_METHOD(SmartsAnalyzerFixture,
                 "SmartsAnalyzer: enumerate_variants max=0",
                 "[SmartsAnalyzer]") {
...
// Test deduplication
TEST_CASE_METHOD(SmartsAnalyzerFixture,
                 "SmartsAnalyzer: enumerate_variants deduplication",
                 "[SmartsAnalyzer]") {
...
TEST_CASE_METHOD(SmartsAnalyzerFixture,
                 "SmartsAnalyzer: enumerate_variants AND", "[SmartsAnalyzer]") {
...
TEST_CASE_METHOD(SmartsAnalyzerFixture,
                 "SmartsAnalyzer: enumerate_variants NOT", "[SmartsAnalyzer]") {
...
TEST_CASE_METHOD(SmartsAnalyzerFixture,
                 "SmartsAnalyzer: enumerate_variants aromatic",
                 "[SmartsAnalyzer]") {
  auto variants = analyzer.enumerate_variants("[c,n]", 10);
...
// Test with bonds
TEST_CASE_METHOD(SmartsAnalyzerFixture,
                 "SmartsAnalyzer: enumerate_variants with bonds",
                 "[SmartsAnalyzer]") {
  auto variants = analyzer.enumerate_variants("[C,N]-[O,S]", 10);
  CHECK(variants.size() == 4);
...
TEST_CASE_METHOD(SmartsAnalyzerFixture,
                 "SmartsAnalyzer: enumerate_variants splits OR bonds",
                 "[SmartsAnalyzer]") {
...
TEST_CASE_METHOD(SmartsAnalyzerFixture,
                 ...
                 "[SmartsAnalyzer]") {
  auto variants =
      analyzer.enumerate_variants("[#6;H0;+0;a;D1,D3&$([#6](:[a])(:[a]))]", 20);
...
TEST_CASE_METHOD(
    SmartsAnalyzerFixture,
    "SmartsAnalyzer: recursive flatten preserves cyclic recursive closure",
    "[SmartsAnalyzer]") {
  auto variants =
      analyzer.enumerate_variants("[#6;$([#6]1=[#6:2]~[#6:3]~1)]", 20);

  REQUIRE_FALSE(variants.empty());

  bool saw_preserved_recursive_cycle = false;
  for (const auto &v : variants) {
    if (v.find("$(") != std::string::npos &&
        (v.find("~1") != std::string::npos ||
         v.find("1=") != std::string::npos ||
         v.find("=1") != std::string::npos)) {
      saw_preserved_recursive_cycle = true;
    }
  }

  CHECK(saw_preserved_recursive_cycle);
}
TEST_CASE_METHOD(
    SmartsAnalyzerFixture,
    "SmartsAnalyzer: standard_smarts preserves explicit ring constraints",
    "[SmartsAnalyzer]") {
  const std::string input = "[o&R1:1]1[n&R1:2][n&R1:3][c&R1:4][c&R1:5]1";

  auto out = analyzer.standard_smarts({input}, false);
  REQUIRE(out.size() == 1);
  INFO("standard_smarts output: " << out[0]);
  CHECK(out[0].find("R1:1") != std::string::npos);
  CHECK(out[0].find("R1:2") != std::string::npos);
  CHECK(out[0].find("R1:3") != std::string::npos);
}

TEST_CASE_METHOD(
    SmartsAnalyzerFixture,
    "SmartsAnalyzer: standard_smarts preserves mapped sulfur endpoint",
    "[SmartsAnalyzer]") {
  const std::string input = "[#16:1]-[N&H2&D1&+0:2]";

  auto out = analyzer.standard_smarts({input}, false);
  REQUIRE(out.size() == 1);
  INFO("standard_smarts output: " << out[0]);

  CHECK(out[0].find(":1") != std::string::npos);
  CHECK(out[0].find(":2") != std::string::npos);
  const bool has_sulfur_token = out[0].find("#16") != std::string::npos ||
                                out[0].find("[S") != std::string::npos;
  CHECK(has_sulfur_token);
}

TEST_CASE_METHOD(
    SmartsAnalyzerFixture,
    "SmartsAnalyzer: standard_smarts keeps O- branch for Sv6X4 pattern",
    "[SmartsAnalyzer]") {
  const std::string input =
      "[#6:1][S&v6&X4:2](=[O:3])(=[O:4])[O&H1,O&-:5]";
  ...
  const bool has_sulfur_token = out[0].find("#16") != std::string::npos ||
                                out[0].find("[S") != std::string::npos;
  CHECK(has_sulfur_token);
  CHECK(out[0].find("&-") != std::string::npos);
}

// Test that variants are all valid SMARTS
TEST_CASE_METHOD(SmartsAnalyzerFixture,
                 "SmartsAnalyzer: enumerate_variants valid output",
                 "[SmartsAnalyzer]") {
  auto variants = analyzer.enumerate_variants("[C,N,O]", 10);
...
TEST_CASE_METHOD(SmartsAnalyzerFixture,
                 "SmartsAnalyzer: enumerate_variants large DOF small max",
                 "[SmartsAnalyzer]") {
  // [C,N,O,S,P][H0,H1,H2,H3][+0,+1,-1] = 5*4*3 = 60 possibilities
  auto variants =
      analyzer.enumerate_variants("[C,N,O,S,P][H0,H1,H2,H3][+0,+1,-1]", 10);
...
// Test charge queries
TEST_CASE_METHOD(SmartsAnalyzerFixture,
                 "SmartsAnalyzer: enumerate_variants charges",
                 "[SmartsAnalyzer]") {
...
TEST_CASE_METHOD(SmartsAnalyzerFixture,
                 "SmartsAnalyzer: enumerate_variants hydrogen counts",
                 "[SmartsAnalyzer]") {
...
// Test empty SMARTS
TEST_CASE_METHOD(SmartsAnalyzerFixture,
                 "SmartsAnalyzer: enumerate_variants empty",
                 "[SmartsAnalyzer]") {
...
TEST_CASE_METHOD(SmartsAnalyzerFixture,
                 "SmartsAnalyzer: enumerate_variants realistic chemistry",
                 "[SmartsAnalyzer]") {
...
TEST_CASE_METHOD(SmartsAnalyzerFixture,
                 "SmartsAnalyzer: consistency DOF vs enumerate",
                 "[SmartsAnalyzer]") {
...
TEST_CASE_METHOD(SmartsAnalyzerFixture,
                 "SmartsAnalyzer: consistency three elements",
                 "[SmartsAnalyzer]") {
